...
> 현재 개발이 진행 중인 프로젝트로, 아래는 설계 방향입니다.

- **프레임워크 없이 Vanilla JS**로 사용자/관리자 화면과 포인트·등급 상태를 직접 관리 — 의존성을 최소화하되 구조가 흐트러지지 않도록 모듈을 기능별로 분리.
- **신뢰 경계를 서버에 유지** — 적립·등급 계산과 데이터 접근 규칙을 클라이언트가 아닌 Supabase(PostgreSQL) 쪽에 두어, 포인트·등급이 클라이언트에서 조작되지 않도록 설계 중.
- **권한 분리** — Supabase Auth로 일반 사용자와 관리자 화면(`admin.html`)의 접근 권한을 분리.
...
관리자 계정은 Supabase Auth에서 직접 생성한 뒤 `profiles.role`을 `admin`으로 설정합니다.
